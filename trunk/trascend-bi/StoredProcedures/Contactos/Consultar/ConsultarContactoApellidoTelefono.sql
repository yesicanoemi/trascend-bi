...
GO
CREATE PROCEDURE [dbo].[ConsultarContactoApellidoTelefono]
...
SET NOCOUNT ON;
	...
	SELECT C.NOMBRE AS NOMBRE, C.APELLIDO AS APELLIDO, C.AREANEGOCIO AS AREA, C.CARGO AS CARGO, T.CODIGOAREA AS CODIGOAREA, T.NUMERO AS NUMERO, T.TIPO AS TIPO
		...
		WHERE C.Apellido= @apellido and T.CodigoArea=@codigoArea 
              and T.Numero=@Numero and T.IdContacto=C.IdContacto
